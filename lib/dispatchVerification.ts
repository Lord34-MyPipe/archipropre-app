...
/** Enveloppe (simulation) : la somme hebdo annoncée doit rester proche de la cible — tolérance 15% (plancher 20 min). */
function verifierEnveloppe(dispatch: DispatchJour[], enveloppeMinutesHebdo: number): string[] {
  const somme = dispatch.reduce((s, j) => s + j.duree_totale_estimee_minutes, 0)
  const tolerance = Math.max(20, enveloppeMinutesHebdo * 0.15)
  const ecart = Math.abs(somme - enveloppeMinutesHebdo)
  if (ecart > tolerance) {
    return [`Dérive d'enveloppe : la proposition totalise ${somme} min/semaine, contre une enveloppe cible de ${Math.round(enveloppeMinutesHebdo)} min (écart ${Math.round(ecart)} min, tolérance ${Math.round(tolerance)} min).`]
  }
  return []
}
...
export function verifierPropositionDispatch(params: {
  ...
  const violations: string[] = [
    ...
    ...(params.enveloppeMinutesHebdo != null ? verifierEnveloppe(params.dispatch, params.enveloppeMinutesHebdo) : []),
  ]
